...
static EC_T_VOID LinkParmsInit(EC_T_LINK_PARMS* pLinkParms, 
                               EC_T_DWORD dwSignature, EC_T_DWORD dwSize, const EC_T_CHAR* szDriverIdent,
                               EC_T_DWORD dwInstance, EC_T_LINKMODE eLinkMode, EC_T_DWORD dwIstPriority = 0)
{
...
#if (defined INCLUDE_XMC)
/***************************************************************************************************/
/**
\brief  Try to create XMC link layer parameters according to current command line parsing

\return EC_E_NOERROR     if link layer parameters was created
        EC_E_NOTFOUND    if command line was not matching
        EC_E_INVALIDPARM if syntax error
*/
static EC_T_DWORD CreateLinkParmsFromCmdLineXMC(EC_T_CHAR** ptcWord, EC_T_CHAR** lpCmdLine, EC_T_CHAR* tcStorage, EC_T_BOOL*  pbGetNextWord,
                                                 EC_T_LINK_PARMS** ppLinkParms)
{
EC_T_DWORD dwRetVal = EC_E_ERROR;
EC_T_LINK_PARMS_XMC* pLinkParmsAdapter = EC_NULL;

    EC_UNREFPARM(pbGetNextWord);

    /* check for matching adapter */
    if (OsStricmp((*ptcWord), "-XMC") != 0)
    {
        dwRetVal = EC_E_NOTFOUND;
        goto Exit;
    }
    /* alloc adapter specific link parms */
    pLinkParmsAdapter = (EC_T_LINK_PARMS_XMC*)OsMalloc(sizeof(EC_T_LINK_PARMS_XMC));
    if (EC_NULL == pLinkParmsAdapter)
    {
        dwRetVal = EC_E_NOMEMORY;
        goto Exit;
    ...
    OsMemset(pLinkParmsAdapter, 0, sizeof(EC_T_LINK_PARMS_XMC));
    LinkParmsInit(&pLinkParmsAdapter->linkParms, EC_LINK_PARMS_SIGNATURE_XMC, sizeof(EC_T_LINK_PARMS_XMC), EC_LINK_PARMS_IDENT_XMC, 1, EcLinkMode_POLLING);
    ...
    {
       EC_T_BYTE abyStationAddress[] = {0x00, 0x00, 0x45, 0x19, 0x05, 0xd5};
       memcpy(pLinkParmsAdapter->abyStationAddress, abyStationAddress, 6);
    }
...
#endif
#if (defined INCLUDE_XMC)
    dwRetVal = CreateLinkParmsFromCmdLineXMC(ptcWord, lpCmdLine, tcStorage, pbGetNextWord, ppLinkParms);
    if (EC_E_NOTFOUND != dwRetVal)
    {
        goto Exit;
    }
...
EC_T_VOID ShowLinkLayerSyntax2(EC_T_VOID)
{
    ...
    OsDbgMsg("     RefBoard        am572x-idk | am571x-idk | am3359-icev2\n");
#endif
#if (defined INCLUDE_L9218I)
...
#endif
#if (defined INCLUDE_ANTAIOS)
    OsDbgMsg("   -antaios          Link layer = Profichip ANTAIOS\n");
#endif
#if (defined INCLUDE_SOCKRAW)
